%%% snmp_collector_rest_res_http.erl
%%% vim: ts=3
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% @doc Body producing function for `GET /snmp/v1/log/http'
%% requests.
get_http() ->
	{ok, MaxItems} = application:get_env(snmp_collector, rest_page_size),
	Log = snmp_collector_log:httpd_logname(transfer),
...
%% @hidden
read_http_log(Log, MaxItems) ->
	case snmp_collector_log:last(Log, MaxItems) of
		{error, _} ->
			{error, 500};
		{NumItems, Events} ->
