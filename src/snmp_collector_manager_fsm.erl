...
%%%     module implements a PDU handler in the
...
handle_pdu(timeout = _Event, #statedata{socket = _Socket, address = Address,
	...
	case catch snmp_pdus:dec_message_only(Packet) of
		...
		{'EXIT', Reason} ->
			error_logger:info_report(["SNMP Manager Decoding SNMP Failed",
						{error, Reason},
						{port, Port},
						{address, Address}]),
					{stop, shutdown, StateData}
...
%% @doc Map flag value to `SecLevel'.
flag(0) ->
	noAuthNoPriv;
flag(1) ->
	authNoPriv;
